...
#image_sets = ['0001.png','0002.png','0003.png']
...
filepath = 'D:\Python\seg-data\data-GF/' 

def get_train_val(val_rate = 0.5):
    train_url = []    
    train_set = []
    val_set  = []
    for pic in os.listdir(filepath + r'src/'):
...
def generateData(batch_size,data=[]):  
    ...
    while True:  
        ...
        for i in (range(len(data))): 
            ...
            img = cv2.imread(filepath + 'src/' + url)
            c1,c2,c3=cv2.split(img)
            img=np.concatenate(([c1],[c2],[c3]),axis=0)
...
def args_parse():
    ...
    ap.add_argument("-m", "--model", required=False,default=filepath+'model/GF-test.h5',
                    help="path to output model")
    ap.add_argument("-p", "--plot", type=str, default=filepath+"model/GF-test.png",
...
if __name__=='__main__':  
    ...
    if args['augment'] == True:
        filepath =r'D:\Python\seg-data\data-GF/'
