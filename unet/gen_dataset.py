...
def creat_dataset(image_num = 50000, mode = 'original'):
    ...
    for i in tqdm(range(len(image_sets))):
        ...
        while count < image_each:
            ...
            cv2.imwrite(('./data/aug/visualize/%d.png' % g_count),visualize)
            cv2.imwrite(('./data/aug/src/%d.png' % g_count),src_roi)
            cv2.imwrite(('./data/aug/label/%d.png' % g_count),label_roi)
            count += 1 
            g_count += 1
